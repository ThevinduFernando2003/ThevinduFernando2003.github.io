    ...
    <div className="mb-16">
      ...
      {items.length === 1 ? (
        <motion.button
          ...
          className="group relative block w-full overflow-hidden border border-border bg-bg-surface text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
        >
          <div className="relative aspect-video">
            {failed[items[0].id] ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-gradient-to-br from-bg-elevated to-bg-surface text-text-dim p-4">
                ...
                <span className="text-xs text-center">{items[0].caption}</span>
              </div>
            ) : (
              ...
              <img
                src={
                  items[0].type === "video"
                    ? getYouTubeThumbnail(items[0].url) ?? items[0].url
                    : items[0].url
                }
                alt={items[0].caption}
                className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                onError={() =>
                  setFailed((f) => ({ ...f, [items[0].id]: true }))
                }
              />
            )}
            ...
            {items[0].type === "video" && (
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="flex h-20 w-20 items-center justify-center rounded-full border border-accent/50 bg-bg/60 text-accent backdrop-blur-sm">
